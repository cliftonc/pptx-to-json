...
describe('PowerPointClipboardProcessor', () => {
  ...
  describe('parseClipboardBuffer', () => {
    it('should throw error for non-Buffer input', async () => {
      await expect(processor.parseClipboardBuffer('not a buffer')).rejects.toThrow('Input must be a Buffer')
      await expect(processor.parseClipboardBuffer(null)).rejects.toThrow('Input must be a Buffer')
      await expect(processor.parseClipboardBuffer(123)).rejects.toThrow('Input must be a Buffer')
    })
    ...
    it('should detect ZIP signature correctly', async () => {
      ...
      // Mock the PowerPointParser to avoid actual parsing
      const mockParseBuffer = vi.spyOn(processor.powerPointParser, 'parseBuffer')
      ...
      expect(mockParseBuffer).toHaveBeenCalledWith(zipBuffer)
    })
    ...
    it('should handle parsing errors gracefully', async () => {
      ...
      const mockParseBuffer = vi.spyOn(processor.powerPointParser, 'parseBuffer')
      mockParseBuffer.mockRejectedValue(new Error('Parsing failed'))
...
describe('PowerPointClipboardProcessor Edge Cases', () => {
  ...
  describe('Buffer validation', () => {
    ...
    it('should handle very large buffer metadata', async () => {
      ...
      vi.spyOn(processor.powerPointParser, 'parseBuffer').mockResolvedValue([])
