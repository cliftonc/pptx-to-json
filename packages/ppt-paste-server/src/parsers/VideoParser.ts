...
export class VideoParser extends BaseParser {
  ...
  static async parseFromNormalized(
    ...
    const { data, spPr, nvPicPr, blipFill, namespace, relationshipId } = videoComponent;
    ...
    const xfrm = VideoParser.safeGet(spPr, 'xfrm');
    const transform = VideoParser.parseTransform(xfrm);

    // Extract component info from nvPicPr
    const cNvPr = VideoParser.safeGet(nvPicPr, 'cNvPr');
    const componentName = VideoParser.safeGet(cNvPr, '$name') || `video-${componentIndex}`;
    const description = VideoParser.safeGet(cNvPr, '$descr') || '';

    // Extract video reference from nvPr
    const nvPr = VideoParser.safeGet(nvPicPr, 'nvPr');
    const videoFile = VideoParser.safeGet(nvPr, 'videoFile');
    const videoRelationshipId = relationshipId || VideoParser.safeGet(videoFile, '$link');
    ...
    if (videoRelationshipId && relationships) {
      ...
      if (!isClipboardFormat && slideIndex !== null) {
        ...
        if (relationships[currentSlideRelFile]) {
          const relsData = VideoParser.safeGet(relationships[currentSlideRelFile], 'Relationships.Relationship') || [];
    ...
    if (blipFill) {
      const blip = VideoParser.safeGet(blipFill, 'blip');
      const thumbnailRelationshipId = VideoParser.safeGet(blip, 'embed');
      ...
      if (thumbnailRelationshipId && relationships && mediaFiles) {
        ...
        if (!isClipboardFormat && slideIndex !== null) {
          ...
          if (relationships[currentSlideRelFile]) {
            const relsData = VideoParser.safeGet(relationships[currentSlideRelFile], 'Relationships.Relationship') || [];
            const rels = Array.isArray(relsData) ? relsData : [relsData];
            const rel = rels.find((r: any) => r && r.$Id === thumbnailRelationshipId);
            ...
            if (rel) {
              ...
              if (thumbnailPath?.startsWith('../')) {
                ...
                thumbnailPath = thumbnailPath.startsWith('media/') ? `ppt/${thumbnailPath}` : thumbnailPath;
              }
            }
          }
        } else {
          // Clipboard format - look directly in relationships
          const slideRelationships = relationships[slideIndex];
          if (slideRelationships && slideRelationships[thumbnailRelationshipId]) {
            const thumbnailRelationship = slideRelationships[thumbnailRelationshipId];
            thumbnailPath = thumbnailRelationship.Target || thumbnailRelationship.target;
          }
        }
        ...
        if (thumbnailPath) {
          ...
          const mediaKey = Object.keys(mediaFiles).find(key => 
            key === thumbnailPath || key.includes(thumbnailPath!) || thumbnailPath!.includes(key.split('/').pop() || '')
          );
          ...
          if (mediaKey && mediaFiles[mediaKey]) {
            ...
            thumbnailSrc = await ImageParser.createImageUrl(mediaData, thumbnailPath, r2Storage);
          }
        }
      }
    }
    ...
    return {
      ...
      metadata: {
        ...
        description,
        videoRelationshipId,
        thumbnailRelationshipId: blipFill ? VideoParser.safeGet(VideoParser.safeGet(blipFill, 'blip'), 'embed') : null,
   ...
   */
  static extractVideoInfo(url: string): { embedType: 'youtube' | 'vimeo' | 'generic', videoId?: string } {
