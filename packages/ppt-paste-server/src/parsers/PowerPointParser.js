...
export class PowerPointParser extends BaseParser {
  ...
  // ====== OLD METHODS (can be removed after testing) ======
   ...
   */
  async extractComponents(json, { debug = false } = {}) {
    const components = [];
    ...
    if (slideFiles.length === 0) {
      ...
      if (debug) console.log('📄 No normal slides found, checking clipboard files:', slideFiles);
    } else {
      if (debug) console.log('📄 Found normal slides:', slideFiles);
    }
    ...
    for (let i = 0; i < slideFiles.length; i++) {
      ...
      if (debug) {
        if (debug) console.log(`🔍 Parsing file ${i + 1}: ${slideFile}`);
        if (debug) console.log(`📄 File structure:`, Object.keys(slideData || {}));
      }
      
      // Handle normal slide structure
      if (slideData && slideData['p:sld']) {
        if (debug) console.log('📄 Processing normal PowerPoint slide');
        ...
        const mediaFiles = this.extractMediaFiles(json, { debug });

        // Handle both array and single object cases for slide data
        let slide = slideData['p:sld'];
      ...
      // Handle any clipboard structure with flexible parsing
        ...
        if (debug) console.log('📄 Processing clipboard structure with flexible parser');
        const clipboardComponents = await this.parseFlexibleClipboard(slideData, json, slideFile, i, { debug });
   ...
   */
  async parseSlide(slide, relationships, mediaFiles, slideIndex, { debug = false } = {}) {
    const components = [];
    let componentIndex = 0;

    // Parse slide content - check common slide layout areas
    // Handle both array and single object cases for cSld
    let cSld = slide['p:cSld'];
    if (Array.isArray(cSld)) {
      cSld = cSld[0];
    } else if (!cSld) {
      cSld = slide; // fallback to slide itself
    }
    
    // Handle both array and single object cases for spTree
    let spTree = cSld['p:spTree'];
    ...
    if (debug) console.log(`✅ Slide ${slideIndex + 1}: found ${components.length} components`);
    return components;
  }
   ...
   */
  extractMediaFiles(json, { debug = false } = {}) {
    const mediaFiles = {};
    ...
    if (debug) console.log('🖼️ Found media files:', Object.keys(mediaFiles));
    return mediaFiles;
  }
   ...
   */
  async parseFlexibleClipboard(slideData, fullJson, slideFile, slideIndex, { debug = false } = {}) {
    const components = [];
    if (debug) console.log('🔍 Attempting flexible clipboard parsing...');

    // Strategy 1: Look for shapes in common locations
    const shapeComponents = await this.findShapesRecursively(slideData, fullJson, slideIndex, { debug });
    if (shapeComponents.length > 0) {
      if (debug) console.log(`✅ Found ${shapeComponents.length} shapes via recursive search`);
    ...
    if (components.length === 0) {
      if (debug) console.log('🔄 No structured components found, trying regex fallback...');
      const regexComponents = await this.parseWithRegexFallback(fullJson, slideFile, { debug });
      if (regexComponents.length > 0) {
        if (debug) console.log(`✅ Found ${regexComponents.length} components via regex fallback`);
    ...
    if (components.length === 0) {
      if (debug) console.log('⚠️ No components found, creating debug info component');
      components.push({
   ...
   */
  async findShapesRecursively(data, fullJson, slideIndex, { debug = false } = {}) {
    const components = [];
    let componentIndex = 0;
    const processedPaths = new Set(); // Track processed paths to avoid duplicates

    const searchForShapes = async (obj, path = '') => {
      ...
      for (const key of Object.keys(obj)) {
        ...
        // If key suggests shapes, pictures, or graphic frames (tables), process the data
        if (key === 'a:sp' || key === 'p:sp' || key === 'xdr:sp' || 
            key === 'a:pic' || key === 'p:pic' || key === 'xdr:pic' ||
            key === 'a:graphicFrame' || key === 'p:graphicFrame' || key === 'xdr:graphicFrame') {
          // Only process if we haven't seen this path before
          if (!processedPaths.has(fullPath)) {
            processedPaths.add(fullPath);
            if (debug) console.log(`🔍 Processing shape/picture/graphic element: ${fullPath}`);
            
            // Handle both array (xml2js) and single object (fast-xml-parser) formats
            const items = Array.isArray(obj[key]) ? obj[key] : [obj[key]];
            if (debug) console.log(`🔍 Found ${items.length} elements: ${fullPath}`);
            ...
            for (let index = 0; index < items.length; index++) {
              ...
              if (debug) console.log(`🔍 Item at ${fullPath}[${index}] keys:`, Object.keys(item || {}));
              
              // If it's a picture element, try parsing as image first
              if (key.includes('pic')) {
                if (debug) console.log(`🖼️ Trying to parse picture element at ${fullPath}[${index}]`);
                const relationships = this.extractClipboardRelationships(fullJson, { debug });
                const mediaFiles = this.extractMediaFiles(fullJson, { debug });
                component = ImageParser.parse(item, relationships, mediaFiles, componentIndex);
                if (debug) console.log(`🖼️ ImageParser.parse returned:`, component ? `${component.type} component` : 'null');
              }
              
              // Also check if the item contains picture-related elements
              if (!component && item && (item['a:pic'] || item['pic'] || item['p:pic'])) {
                if (debug) console.log(`🖼️ Found picture element in item structure at ${fullPath}[${index}]`);
                const relationships = this.extractClipboardRelationships(fullJson, { debug });
                const mediaFiles = this.extractMediaFiles(fullJson, { debug });
                component = ImageParser.parse(item, relationships, mediaFiles, componentIndex);
                if (debug) console.log(`🖼️ ImageParser.parse returned:`, component ? `${component.type} component` : 'null');
              ...
              // If it's a graphic frame element, try parsing as table or other graphic
              if (!component && key.includes('graphicFrame')) {
                if (debug) console.log(`📊 Trying to parse as graphic frame (table) at ${fullPath}[${index}]`);
                const relationships = this.extractClipboardRelationships(fullJson, { debug });
                const mediaFiles = this.extractMediaFiles(fullJson, { debug });
                component = await this.parseGraphicFrame(item, relationships, mediaFiles, componentIndex, { debug });
                if (debug) console.log(`📊 GraphicFrame parsing returned:`, component ? `${component.type} component` : 'null');
              }
              
              // If picture parsing failed, check if it's a text shape (has a:txSp)
              if (!component && item['a:txSp']) {
                if (debug) console.log(`📝 Trying to parse as text component at ${fullPath}[${index}]`);
                component = TextParser.parse(item, componentIndex);
              }
              
              // If still no component, try parsing as regular shape
              if (!component) {
                component = this.tryParseAsShape(item, componentIndex, `${fullPath}[${index}]`, { debug });
              }
   ...
   */
  tryParseAsShape(shapeData, index, path, { debug = false } = {}) {
    try {
      if (!shapeData || typeof shapeData !== 'object') return null;

      if (debug) console.log(`🎯 Trying to parse shape at ${path}`);
      if (debug) console.log(`🔍 Shape keys:`, Object.keys(shapeData));
      ...
      if (hasText) {
        if (debug) console.log(`📝 Parsing as text component`);
      ...
      if (debug) console.log(`🔸 Parsing as shape component with improved styling`);
      ...
      if (debug) console.log(`🔧 Using manual parsing fallback`);
      ...
      if (debug) console.log(`🔍 Analyzing shape data for colors...`);
        ...
        if (solidFill && solidFill[0]) {
          ...
          if (srgbClr && srgbClr[0] && srgbClr[0].$.val) {
            ...
            if (colorMap[schemeVal]) {
              ...
              if (debug) console.log(`🔧 Extracted scheme color: ${schemeVal} -> ${backgroundColor}`);
            } else {
              if (debug) console.log(`🔧 Ignoring scheme color: ${schemeVal} (keeping transparent)`);
              backgroundColor = 'transparent';
            }
          }
        } else {
          if (debug) console.log(`🔧 No solid fill found, keeping transparent`);
        }
        
        // Try to extract border color
        const line = spPr[0]['a:ln'] || spPr[0]['p:ln'];
        if (debug) console.log(`🔍 Checking for border:`, !!line);
        
        if (line && line[0]) {
          const lineFill = line[0]['a:solidFill'] || line[0]['p:solidFill'];
          if (debug) console.log(`🔍 Border has fill:`, !!lineFill);
          ...
          if (lineFill && lineFill[0]) {
            ...
            if (debug) console.log(`🔍 Border color data: scheme=${!!schemeClr}, rgb=${!!srgbClr}`);
            
            if (srgbClr && srgbClr[0] && srgbClr[0].$.val) {
              // Prefer explicit RGB colors over scheme colors
              borderColor = '#' + srgbClr[0].$.val.toLowerCase();
              if (debug) console.log(`🔧 Extracted border RGB color: ${borderColor}`);
            } else if (schemeClr && schemeClr[0] && schemeClr[0].$.val) {
              const schemeVal = schemeClr[0].$.val;
              if (debug) console.log(`🔍 Found border scheme color: ${schemeVal}`);
              ...
              if (colorMap[schemeVal]) {
                ...
                if (debug) console.log(`🔧 Extracted border scheme color: ${schemeVal} -> ${borderColor}`);
              } else {
                if (debug) console.log(`🔧 Ignoring border scheme color: ${schemeVal} (keeping transparent)`);
                borderColor = 'transparent';
              }
            }
          }
        } else {
          if (debug) console.log(`🔧 No border found, keeping transparent`);
        }
      }
   ...
   * @returns {Promise<Array>} parsed components
   */
  async parseWithRegexFallback(fullJson, targetFile, { debug = false } = {}) {
    if (debug) console.log('🔄 Using regex fallback parser...');
   ...
   */
  async parseClipboardDrawing(drawing, fullJson, drawingIndex, { debug = false } = {}) {
    const components = [];
    let componentIndex = 0;

    if (debug) console.log('🎨 Parsing clipboard drawing structure');
   ...
   */
  async parseGenericStructure(data, fullJson, dataIndex, { debug = false } = {}) {
    const components = [];
    let componentIndex = 0;

    if (debug) console.log('🎨 Attempting generic structure parse');
    ...
    const findElements = (obj, path = '') => {
      ...
      Object.keys(obj).forEach(key => {
        ...
        if (key.includes('sp') || key.includes('shape')) {
          if (debug) console.log(`🔍 Found potential shape at: ${fullKey}`);
        ...
        if (key.includes('text') || key.includes('txBody')) {
          if (debug) console.log(`🔍 Found potential text at: ${fullKey}`);
        }
   ...
   */
  extractClipboardRelationships(json, { debug = false } = {}) {
    const relationships = {};
    ...
    if (debug) console.log('🔗 Found relationship files:', Object.keys(relationships));
   ...
   */
  async linkImagesToShapes(components, mediaFiles, relationships, { debug = false } = {}) {
    if (debug) console.log('🔗 Linking images to shapes using relationships...');
    ...
    for (const component of components) {
      if (component.type === 'shape' || component.type === 'text') {
        ...
        if (linkedImageComponent) {
          if (debug) console.log(`🔗 Linked shape ${component.id} to image, converting to image component`);
    ...
    if (debug) console.log(`🔗 Processed ${components.length} components, ${linkedMediaFiles.size} images linked`);
        ...
        relArray.forEach(rel => {
          if (rel.$ && rel.$.Id && rel.$.Target) {
            ...
            if (target.includes('media/')) {
              ...
              if (debug) console.log(`🔗 Mapped relationship ${rId} -> ${mediaPath}`);
            }
          }
        });
      }
  ...
  async tryLinkComponentToImage(component, mediaFiles, relationshipMappings, linkedMediaFiles) {
    ...
    if (mediaFileKeys.length === 1 && !linkedMediaFiles.has(mediaFileKeys[0])) {
      ...
      if (isBufferLike(mediaFile)) {
        ...
        if (debug) console.log(`✅ Linking ${component.type} component to image: ${filename} (${mediaFile.length} bytes)`);
   ...
   */
  async findOrphanedMediaFiles(mediaFiles, relationships, existingComponents, slideIndex, fullJson, { debug = false } = {}) {
    const orphanedImages = [];
    const mediaKeys = Object.keys(mediaFiles);

    if (mediaKeys.length === 0) {
      if (debug) console.log('📷 No media files to check for orphans');
      return orphanedImages;
    }

    if (debug) console.log(`📷 Checking ${mediaKeys.length} media files for orphans...`);
    ...
    if (debug) console.log(`📷 Found ${usedRelationshipIds.size} relationship IDs already used by proper components`);
    ...
    for (let i = 0; i < mediaKeys.length; i++) {
      ...
      if (relId && usedRelationshipIds.has(relId)) {
        if (debug) console.log(`📷 Skipping ${mediaKey} - already used by properly parsed component (${relId})`);
        continue;
      }

      if (debug) console.log(`📷 Processing orphaned media file: ${mediaKey} (${mediaFile.length} bytes)`);
      
      // IMPORTANT: Let's see if the PowerPoint dimension search is working
      if (debug) console.log(`🔍 DEBUG: About to call findImageDimensionsFromShapes for ${mediaKey}`);
      ...
      const powerPointDimensions = await this.findImageDimensionsFromShapes(mediaKey, fullJson, { debug });
      
      let imageDimensions;
      if (powerPointDimensions) {
        imageDimensions = powerPointDimensions;
        if (debug) console.log(`📷 Using PowerPoint-scaled dimensions: ${imageDimensions.width}x${imageDimensions.height} (from shape transform)`);
      } else {
        // Fallback to raw image dimensions
        imageDimensions = this.extractImageDimensions(mediaFile, imageType);
        if (debug) console.log(`📷 Using raw file dimensions: ${imageDimensions.width}x${imageDimensions.height} (no PowerPoint scaling found)`);
      }
      ...
      if (debug) console.log(`✅ Created orphaned image component: ${filename} (${mediaFile.length} bytes)`);
    }
   ...
   */
  async findImageDimensionsFromShapes(mediaKey, fullJson, { debug = false } = {}) {
    try {
      if (debug) console.log(`🔍 Searching for PowerPoint dimensions for ${mediaKey}...`);
          ...
          relArray.forEach(rel => {
            if (rel.$ && rel.$.Id && rel.$.Target) {
              ...
              if (normalizedTarget === mediaKey) {
                targetRId = rel.$.Id;
                if (debug) console.log(`🔍 Found relationship ${targetRId} for ${mediaKey}`);
              }
            }
          });
        }
      ...
      if (!targetRId) {
        if (debug) console.log(`❌ No relationship ID found for ${mediaKey}`);
        return null;
      }
      ...
      for (const drawingFile of drawingFiles) {
        if (debug) console.log(`🔍 Searching ${drawingFile} for r:embed=${targetRId}...`);
        const dimensions = await this.searchDrawingForImageDimensions(fullJson[drawingFile], targetRId);
        
        if (dimensions) {
          if (debug) console.log(`✅ Found PowerPoint dimensions in ${drawingFile}: ${dimensions.width}x${dimensions.height}`);
          return dimensions;
        }
      }
      
      if (debug) console.log(`❌ No PowerPoint dimensions found for ${mediaKey} with r:embed=${targetRId}`);
      return null;
      ...
      const searchForBlipFill = (obj, path = '') => {
        ...
        if (obj['a:blipFill']) {
          ...
          for (const blipFill of blipFillArray) {
            ...
            if (rId === targetRId) {
              if (debug) console.log(`🎯 Found matching r:embed=${targetRId} at ${path}`);
   ...
   */
  findTransformInParent(shapeObj, path, { debug = false } = {}) {
    try {
      ...
      for (const transformPath of possibleTransformPaths) {
        ...
        if (xfrm) {
          if (debug) console.log(`🔍 Found transform at ${path}.${transformPath}`);
          const transform = BaseParser.parseTransform(xfrm);
          
          if (transform.width > 0 && transform.height > 0) {
            if (debug) console.log(`✅ Extracted PowerPoint dimensions: ${transform.width}x${transform.height}`);
            return { width: transform.width, height: transform.height };
          }
        }
      }
      
      if (debug) console.log(`❌ No valid transform found at ${path}`);
      return null;
